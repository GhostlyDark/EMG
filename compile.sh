...
if [[ ! $(uname -s) = Darwin* && ! -f "$emg_dir"/EMG$exe ]] || [[ $(uname -s) = Darwin* && ! -d "$emg_dir"/EMG.app ]]; then
    ...
    if [[ $(uname -s) = MINGW64* ]]; then
        ...
        "$tools_dir"/rcedit-x64 "$emg_dir"/EMG.exe --set-icon "$ico_dir"/emg.ico --set-version-string LegalCopyright "(C) 2026 EvilGames.eu" --set-version-string OriginalFilename "electron.exe" --set-version-string FileDescription "EMG" --set-version-string ProductName "EMG" --set-version-string CompanyName "EvilGames.eu"
